...
import com.honeywell.hch.airtouchv3.framework.enrollment.controls.EnrollmentClient;
import com.honeywell.hch.airtouchv3.framework.enrollment.models.DIYInstallationState;
import com.honeywell.hch.airtouchv3.framework.enrollment.models.http.WAPIDeviceResponse;
import com.honeywell.hch.airtouchv3.framework.enrollment.models.http.WAPIErrorCodeResponse;
import com.honeywell.hch.airtouchv3.framework.enrollment.models.http.WAPIKeyResponse;
...
import com.honeywell.hch.airtouchv3.framework.webservice.task.AddDeviceTask;
import com.honeywell.hch.airtouchv3.framework.webservice.task.AddLocationTask;
import com.honeywell.hch.airtouchv3.framework.webservice.task.CheckMacTask;
...
public class EnrollDeviceManager implements WifiUtil.IWifiOpen {
    ...
    private List<UserLocationData> mUserLocations;
    private UserLocationData mSelectedHomeWaitingForAdd;
    ...
    final IReceiveResponse getWAPIResponse = new IReceiveResponse() {
        ...
        public void onReceive(HTTPRequestResponse httpRequestResponse) {
            if (httpRequestResponse.getStatusCode() == StatusCode.OK) {
                ...
                mErrorCallback.onError(null, mContext.getResources().getString(R.string.enroll_loading_fail));
            }
        }
    };
                    ...
                    switch (responseResult.getRequestId()) {
                        case ADD_LOCATION:
                            if (responseResult.getResponseCode() == StatusCode.OK) {
                                ...
                                addDevice();
                                sendBroadcastToMainPage(true);
                            } else {
                                mErrorCallback.onError(responseResult, mContext.getString(R.string.enroll_loading_fail));
                            }
                            return;
                        default:
                            break;
...
//        mLoadingCallback.onLoad(mContext.getString(R.string.adding_device));
    ...
    private int getLocationIdFromHomeName() {
        mUserLocations = AppManager.shareInstance().getUserLocationDataList();
        if (mUserLocations.size() > 0) {
            for (int i = 0; i < mUserLocations.size(); i++) {
                if (mUserLocations.get(i).getName().equals(mHomeName)) {
                    isDeviceNumberReachMax = mUserLocations.get(i).getHomeDevicesList().size()
