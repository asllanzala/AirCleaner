...
public class LocationBaiduUtils {
    ...
    public void beginRequestLocation() {
        mLocationClient = new LocationClient(HPlusApplication.getInstance()); // 声明LocationClient类
...
//		option.setPriority(LocationClientOption.GpsFirst); // 不设置，默认是gps优先
        if (Build.VERSION.SDK_INT  >= Build.VERSION_CODES.M && android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.M){
            HPlusApplication.getInstance().getApplicationContext().checkSelfPermission(Manifest.permission.WRITE_SETTINGS);
        }
