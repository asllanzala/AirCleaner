package com.honeywell.hch.airtouchv3.framework.view.residemenu;
...
public class ResideMenu extends FrameLayout {
    ...
    private float mScaleValueY = 0.8f;
    ...
    private void initViews(Context context) {
        ...
        versionTextView.setText(getVersion(context));
    ...
    public void openMenu(int direction) {
        ...
        AnimatorSet scaleUP_menu = buildScrollViewScaleUpAnimation(scrollViewMenu, 1.0f, 1.0f);
    ...
    public void closeMenu() {
        ...
        if (scrollViewMenu != null){
            AnimatorSet scaleDown_menu = buildScrollViewScaleDownAnimation(scrollViewMenu, 0.1f, 1f);
            scaleUp_activity.playTogether(scaleDown_menu);
        }
        if (imageViewShadow != null || scrollViewMenu != null){
    ...
    private void setScaleDirection(int direction) {
        ...
        float pivotX;
        float pivotY = getScreenHeight() * (1 - mScaleValueY) * 2.0f;
        float menuPivotX;
        float menuPivotY = getScreenHeight() / 2;

        if (direction == DIRECTION_LEFT) {
            scrollViewMenu = scrollViewLeftMenu;
            pivotX = screenWidth * 1.5f;
            menuPivotX = screenWidth * -0.5f;
        } else {
            scrollViewMenu = scrollViewRightMenu;
            pivotX = screenWidth * -0.5f;
            menuPivotX = screenWidth;
        }
        scrollViewMenu.getLayoutParams().width = (int) (getScreenWidth() * (1 - mScaleValueX / 2));

        ViewHelper.setPivotX(viewActivity, pivotX);
        ViewHelper.setPivotY(viewActivity, pivotY);
        ViewHelper.setPivotX(imageViewShadow, pivotX);
        ViewHelper.setPivotY(imageViewShadow, pivotY);
//        ViewHelper.setPivotX(scrollViewMenu, pivotX);
        ViewHelper.setPivotX(scrollViewMenu, menuPivotX);
        ViewHelper.setPivotY(scrollViewMenu, menuPivotY);
    ...
    private AnimatorSet buildScaleDownAnimation(View target, float targetScaleX, float targetScaleY) {
        ...
        scaleDown.playTogether(
                ...
                ObjectAnimator.ofFloat(target, "scaleX", targetScaleY),
                ObjectAnimator.ofFloat(target, "translationX", 0,getScreenWidth()*0.45f)
     ...
     * @return
     */
    private AnimatorSet buildScrollViewScaleDownAnimation(View target, float targetScaleX, float targetScaleY) {
        AnimatorSet scaleDown = new AnimatorSet();
        scaleDown.playTogether(
                ObjectAnimator.ofFloat(target, "scaleY", targetScaleY),
    ...
    private AnimatorSet buildScaleUpAnimation(View target, float targetScaleX, float targetScaleY) {
        ...
        scaleUp.playTogether(
                ObjectAnimator.ofFloat(target, "scaleX", targetScaleX),
                ObjectAnimator.ofFloat(target, "scaleY", targetScaleY),
                ObjectAnimator.ofFloat(target, "translationX", getScreenWidth() * 0.45f, 0)
     ...
     * @return
     */
    private AnimatorSet buildScrollViewScaleUpAnimation(View target, float targetScaleX, float targetScaleY) {
        AnimatorSet scaleUp = new AnimatorSet();
        scaleUp.playTogether(
                ObjectAnimator.ofFloat(target, "scaleX", targetScaleX),
                ObjectAnimator.ofFloat(target, "scaleY", targetScaleY)
    ...
    private float getTargetScaleX(float currentRawX) {
        float scaleFloatX = ((currentRawX - lastRawX) / getScreenWidth()) * (1 - mScaleValueX) *
                1.5f;
    ...
    private float getTargetScaleY(float currentRawX) {
        float scaleFloatY = ((currentRawX - lastRawX) / getScreenWidth()) * (1 - mScaleValueY) *
                1.5f;
        scaleFloatY = scaleDirection == DIRECTION_RIGHT ? -scaleFloatY : scaleFloatY;

        float targetScale = ViewHelper.getScaleY(viewActivity) - scaleFloatY;
        targetScale = targetScale > 1.0f ? 1.0f : targetScale;
        targetScale = targetScale < mScaleValueY ? mScaleValueY : targetScale;
        return targetScale;
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                // lastActionDown - left side px to drag
                if ((lastActionDownX > 10 && xOffset > 0) || (lastActionDownX < (getScreenWidth()
                        - getScreenWidth() * mScaleValueX / 2) && xOffset < 0))
                    break;
...
//                        showScrollViewMenu(scrollViewMenu);
                    ...
                    float targetScaleX = getTargetScaleX(ev.getRawX());
                    float targetScaleY = getTargetScaleY(ev.getRawX());
                    ViewHelper.setScaleX(viewActivity, targetScaleX);
                    ViewHelper.setScaleY(viewActivity, targetScaleY);
                    ViewHelper.setScaleX(scrollViewMenu, (1 - targetScaleX) * 2.0f);
                    ViewHelper.setScaleY(scrollViewMenu, (1 - targetScaleX) * 2.0f);

                    ViewHelper.setScaleX(imageViewShadow, targetScaleX + shadowAdjustScaleX);
                    ViewHelper.setScaleY(imageViewShadow, targetScaleY + shadowAdjustScaleY);
    ...
    public void setResideMenuScroll(boolean isScroll)
    {
