...
public class UserDBService extends DBService {
    ...
    public static String[] DBKey = {USER_ID, PHONE_NUMBER, PASSWORD, NICKNAME, IS_DEFAULT, IS_ENCRYPTED, COUNTRY_CODE};
    ...
    public synchronized ArrayList<User> findAllUsers() {
    ...
    public synchronized ArrayList<User> getUsersByKey(String key) {
        ArrayList<User> userList = new ArrayList<>();
        SQLiteDatabase sqLiteDatabase = mySQLiteHelper.getReadableDatabase();
        Cursor cursor = sqLiteDatabase.rawQuery("SELECT *"
                + " FROM " + TABLE_NAME
