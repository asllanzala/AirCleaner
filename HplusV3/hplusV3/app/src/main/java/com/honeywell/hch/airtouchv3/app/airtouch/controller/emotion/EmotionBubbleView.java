...
public class EmotionBubbleView extends View
{
    ...
    private static final int MEIZU_SCEEN_HEIGHT = 1800;

    private static final int MIN_SUPER_SOLUTION_RADIO = 2200;
    ...
    private void initMesh()
    {
        ...
        if (DensityUtil.getScreenHeight() == MEIZU_SCEEN_HEIGHT)
        {
            resourceId = R.drawable.meizu_bubble_closedangle;
        }
        else if(DensityUtil.getScreenHeight() >= MIN_SUPER_SOLUTION_RADIO){
            resourceId = R.drawable.emotion_bubble_big;
        }
        mBitmap = BitmapUtil.createBitmapEffectly(mContext, resourceId);
