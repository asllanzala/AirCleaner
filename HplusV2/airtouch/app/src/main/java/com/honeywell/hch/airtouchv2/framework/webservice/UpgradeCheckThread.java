...
import com.honeywell.hch.airtouchv2.lib.util.JSONSerializer;
...
public class UpgradeCheckThread extends AsyncTask<String, Integer, VersionCollector> {
	...
	protected VersionCollector doInBackground(String... params) {
		try {
//			if (ATApplication.isTimeForUpgrade())
//			{
				// Check upgrade
...
//		if(ATApplication.canUpgrade(versionData) && ATApplication.isTimeForUpgrade())
//		{
			//推送显示版本更新的对话框
            boolean installSuccess = false;
            VersionCollector preVersionCollector = ATApplication.getVersionCollector();
            ...
            if(preDownloadStatus == DownloadManager.STATUS_SUCCESSFUL && curFileNameApk.equals(preFileNameApk))
            {
                ...
                // 下载成功，但是上次没有安装
                installSuccess = DownloadUtils.getInstance().install(ATApplication.getInstance(), preDownloadId, ATApplication.getVersionCollector().version_name);
            }
            ...
            if(!installSuccess)
            {
                ...
                ATApplication.setVersionCollector(versionData);
                // 还没下载，开始去下载PK
                long downloadId = DownloadUtils.getInstance().startDownload(versionData);
                // 把新的downloadId存储起来
                ATApplication.getVersionCollector().setDownloadId(downloadId);
//                ATApplication.persistVersionCollector();
            }
			return true;
//		}
//		return false;
